...
const getPanels = (s) => ({
    ...
    16: {
        ...
        desc: `Admin and configuration utility commands for ${(s.botName || "Firebox Bot")}.`,
        status: "",
...
module.exports = {
    ...
    execute: async (ctx) => {
        ...
        if (args.length === 0 || parseInt(args[0]) === 0) {
            const s = settings;
            let menu  = `⚙️ *${(s.botName || "Firebox Bot").toUpperCase()} SETTINGS*\n`;
            menu += `${"─".repeat(30)}\n\n`;
            menu += `_Reply with \`.settings <number>\` (e.g. \`.settings 4\`) to configure:_\n\n`;
            menu += `1. 🤖 Bot Configuration — Name: ${s.botName || "Firebox Bot"} | Mode: ${s.publicMode ? "public" : "private"}\n`;
