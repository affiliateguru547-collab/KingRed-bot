...
test("admin access uses the configured passcode only", () => {
    const previous = process.env.KINGRED_ADMIN_PASSCODE;
    process.env.KINGRED_ADMIN_PASSCODE = "correct-admin-passcode";
    ...
    if (previous === undefined) delete process.env.KINGRED_ADMIN_PASSCODE; else process.env.KINGRED_ADMIN_PASSCODE = previous;
});
