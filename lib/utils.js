/**
 * Centralized utility functions for Kingred Bot
 */

/**
 * Converts a raw number string to a standard WhatsApp JID
 * @param {string} number - The raw number (e.g., "25470...")
 * @returns {string} - The formatted JID (e.g., "25470...@s.whatsapp.net")
 */
const toJid = (number) => {
    if (!number) return "";
    ...
    // Username mappings
    const usernameMap = {
        "fireboxstudios": "254100969922@s.whatsapp.net"
    };
...
const sendButtonMessage = async (sock, jid, text, footerText, buttons = [], media = null, quoted = null) => {
    ...
    try {
        ...
        const newsletterJid = sock.newsletterJid || global.newsletterJid;
        const newsletterName = sock.newsletterName || global.newsletterName || "Kingred Bot Updates";
            ...
            viewOnceMessage: {
                message: {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        contextInfo: (!settings.hideViewChannel && newsletterJid) ? {
                            forwardingScore: 999,
                            isForwarded: true,
