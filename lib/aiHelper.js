/**
 * Shared AI helper for Kingred Bot
...
async function askAI(userPrompt, system = "You are Firebox, a helpful, friendly WhatsApp assistant.") {
    ...
    system = cleanPrompt(system);
    if (!userPrompt) throw new Error("AI prompt cannot be empty.");

    // 1. Try Groq (Primary for speed) — use current supported model IDs.
    if (groqKey) {
        const groqModels = [
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        for (const model of models) {
            try {
                ...
                // Pollinations may return a perfectly valid answer containing the
                // word "error" (for example, when explaining error handling).
                // Validate the response shape instead of scanning its contents.
...
async function askGroq(userPrompt, system = "You are Firebox, a helpful WhatsApp assistant. Reply clearly and briefly.") {
    const key = (process.env.GROQ_API_KEY || groqKey || "").trim();
    if (!key) throw new Error("GROQ_API_KEY is not configured in the bot service.");
    ...
    for (const model of [...new Set(models)]) {
        try {
            const { data } = await axios.post("https://api.groq.com/openai/v1/chat/completions", {
                model,
                messages: [{ role: "system", content: system }, { role: "user", content: String(userPrompt).slice(0, 4000) }],
                temperature: 0.7,
                max_tokens: 500,
            }, {
            ...
            lastError = error;
            console.error(`Groq command mode [${model}] failed:`, error.response?.data?.error?.message || error.message);
        }
    }
    throw new Error(`Groq unavailable: ${lastError?.response?.data?.error?.message || lastError?.message || "all models failed"}`);
}

module.exports = { askAI, askGroq, generateImage, checkAILimit };
