...
const handleAutomation = async (sock, m) => {
    try {
        ...
        if (m.message && !m.key.fromMe) {
            ...
            if (hasGroupPresence || hasDmPresence || hasAutoType || hasAutoRecord) {
                if (!sock.presenceTracker) sock.presenceTracker = {};
                const now = Date.now();
                const last = sock.presenceTracker[jid] || 0;
                if (now - last > 15000) { // Only send once every 15 seconds
                    sock.presenceTracker[jid] = now;
            ...
            if (settings.alwaysOnline) {
                if (!sock.alwaysOnlineTracker) sock.alwaysOnlineTracker = 0;
                const now = Date.now();
                if (now - sock.alwaysOnlineTracker > 15000) {
                    sock.alwaysOnlineTracker = now;
        ...
        if (jid === "status@broadcast") {
            ...
            // Anti-status-mention protection. Baileys exposes status mentions
            // on WebMessageInfo.statusMentions; some versions also place them
            // in statusMentionMessageInfo or extendedTextMessage contextInfo.
            const botJid = (sock.user?.id || sock.myJid || "").split(":")[0];
            const statusMentionJids = [
                ...(Array.isArray(m.statusMentions) ? m.statusMentions : []),
                ...(Array.isArray(m.statusMentionMessageInfo?.mentionedJid) ? m.statusMentionMessageInfo.mentionedJid : []),
                ...(Array.isArray(m.message?.extendedTextMessage?.contextInfo?.mentionedJid)
                    ? m.message.extendedTextMessage.contextInfo.mentionedJid : [])
            ].map(value => String(value).split(":")[0]);
            const botWasMentioned = botJid && statusMentionJids.some(value => value === botJid);
            const antiStatusMode = settings.antiStatusMentionGlobal || "off";

            if (botWasMentioned && antiStatusMode !== "off") {
                console.log(`🏷️ Anti-status-mention detected: mode=${antiStatusMode}, author=${m.key.participant || "unknown"}`);
                if (antiStatusMode === "delete") {
                    try {
                        // This succeeds for statuses owned by the bot. WhatsApp
                        // does not permit deleting another user's status; retain
                        // a clear log instead of silently pretending it worked.
                        await sock.sendMessage("status@broadcast", { delete: m.key });
                        console.log("🗑️ Mentioned status deletion requested.");
                    } catch (error) {
                        console.error("⚠️ Could not delete mentioned status (WhatsApp may restrict deleting another user's status):", error.message);
                    }
                }
                if (antiStatusMode === "warn" || antiStatusMode === "delete" || antiStatusMode === "remove") {
                    const ownerJid = process.env.SUDO || settings.ownerNumber || "";
                    if (ownerJid) {
                        await sock.sendMessage(ownerJid.includes("@") ? ownerJid : `${ownerJid}@s.whatsapp.net`, {
                            text: `🏷️ *Anti-Status-Mention*\nMode: *${antiStatusMode.toUpperCase()}*\nAuthor: ${m.key.participant || "Unknown"}\n\nThe status mentioned this bot.`
                        }).catch(error => console.error("⚠️ Anti-status owner notice failed:", error.message));
                    }
                }
            }
    ...
    if (isGroup && settings.antiSpam && !m.key.fromMe) {
        ...
        if (!sock.spamTracker) sock.spamTracker = {};
        const now = Date.now();
        const key = `${jid}_${sender}`;

        if (!sock.spamTracker[key]) sock.spamTracker[key] = { count: 0, last: now };
        
        const userData = sock.spamTracker[key];
