...
const handleAutomation = async (sock, m) => {
    try {
        ...
        // A status mention is delivered to the bot as a normal chat message in
        // the group where it was shared. Handle that message before the
        // status-feed branch below; deleting the group copy does not attempt
        // to delete the original user's status.
        const botNumber = (sock.user?.id || sock.myJid || "").split(":")[0].split("@")[0];
        const statusMentions = Array.isArray(m.statusMentions) ? m.statusMentions : [];
        const statusMentionInfo = m.statusMentionMessageInfo || m.message?.statusMentionMessage;
        const mentionedBot = statusMentions.some(value => String(value).split(":")[0].split("@")[0] === botNumber);
        const isGroupStatusMention = jid.endsWith("@g.us") &&
            (m.isMentionedInStatus === true || Boolean(statusMentionInfo) || mentionedBot);
        if (isGroupStatusMention) {
            ...
            if (activeMode !== "off") {
                console.log(`🏷️ Group status mention detected: mode=${activeMode}, message=${m.key.id}`);
                if (activeMode === "delete") {
                    try {
                        await sock.sendMessage(jid, { delete: m.key });
                        console.log(`🗑️ Deleted group status mention message ${m.key.id}`);
                    } catch (error) {
                        console.error("⚠️ Failed to delete group status mention message:", error.message);
                    }
                    return;
                }
                ...
                if (ownerJid) {
                    await sock.sendMessage(ownerJid.includes("@") ? ownerJid : `${ownerJid}@s.whatsapp.net`, {
                        text: `🏷️ *Anti-Status-Mention*\nMode: *${activeMode.toUpperCase()}*\nGroup: ${jid}\n\nA group message mentioned the bot in a status.`
