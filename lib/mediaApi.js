...
const mediaApi = {
    ...
    /**
     * Download a public social-media video with the local yt-dlp binary.
     * Used as a fallback when a third-party API is unavailable.
     */
    async ytDlpSocialDownload(url) {
        const workDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'firebox-social-'));
        const outputTemplate = path.join(workDir, 'media.%(ext)s');
        const args = [
            '--no-playlist', '--no-warnings', '--no-progress', '--force-ipv4',
            '--js-runtimes', 'node', '--retries', '2', '--fragment-retries', '2',
            '--print', 'after_move:title', '-f', 'best[ext=mp4]/best',
        ...
        try {
            ...
            const files = (await fs.promises.readdir(workDir)).filter(file => file !== 'media.info.json');
            if (!files.length) throw new Error('yt-dlp returned no media file.');
            const mediaPath = path.join(workDir, files[0]);
            const stat = await fs.promises.stat(mediaPath);
            if (stat.size > MAX_MEDIA_BYTES) throw new Error('Media is too large to send on WhatsApp.');
            const buffer = await fs.promises.readFile(mediaPath);
            const title = stdout.trim().split(/\r?\n/).filter(Boolean).pop() || 'Social media video';
            return { buffer, title, type: 'video' };
        } catch (error) {
            console.error('yt-dlp social download error:', error.stderr || error.message);
            return null;
        } finally {
            await fs.promises.rm(workDir, { recursive: true, force: true }).catch(() => {});
        }
    ...
    async facebookDownload(url) {
        ...
        return this.ytDlpSocialDownload(url);
    },
    ...
    async igDownload(url) {
        ...
        const fallback = await this.ytDlpSocialDownload(url);
        return fallback ? [{ ...fallback, isVideo: true }] : null;
    },
