...
async function handleMessages(sock, { messages, type }) {
    ...
    const sender = msg.key.fromMe ? (sock.myJid || global.myJid || msg.key.remoteJid) : (msg.key.participant || msg.key.remoteJid);
    ...
    if (prefix) {
        ...
        const isReplyToBot = quotedParticipant === (sock.myJid || global.myJid || "") || msg.key.fromMe;
    ...
    if (!command) {
        ...
        // AI auto-replies are private-chat only by default. The owner can opt
        // into all-chat mode with `.chatbot all on`.
        const isPrivateChat = jid.endsWith("@s.whatsapp.net");
        const isPlainIncomingText = !msg.key.fromMe && !prefix && !listResponse && text.trim().length > 0;
        const chatbotEnabledForChat = settings?.chatbotAI && (isPrivateChat || settings?.chatbotAIAll === true);
        if (chatbotEnabledForChat && isPlainIncomingText) {
            ...
            try {
                const reply = await askGroq(text, "You are Firebox Bot, a helpful WhatsApp assistant. Answer the user's message directly, do not claim to be human, and keep the response concise and safe.");
                await sock.sendMessage(jid, { text: reply }, { quoted: msg });
            } catch (error) {
                console.error(`⚠️ Groq auto-reply failed for ${jid}:`, error.message);
            }
        }
        return;
    }
    ...
    if (isGroup && !isOwner) {
        if (!sock.stormTracker) sock.stormTracker = {};
        const now = Date.now();
        if (!sock.stormTracker[jid]) sock.stormTracker[jid] = { count: 0, last: now, mutedUntil: 0 };
